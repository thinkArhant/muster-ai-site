# Sprint Archive
<!-- Completed sprints are moved here from current-sprint.md at sprint closeout. -->
<!-- This file is not read at startup — it's historical reference only. -->
<!-- Format: copy the full sprint section (header, goal, task board, summary) from current-sprint.md. -->
<!-- Newest sprint at the top. -->

## Sprint 1 — CLOSED 2026-07-26

**Delivered**: the page shell in both themes, and §2 — the two-layer annotated replay of Bodh's
Sprint-4 website wave, built from the founder-supplied corpus and playing to a 48.00 s schedule.

**Outcome**: accepted at gate 3 with one carried defect (the key-beat tick collides with the timestamp;
folded into Sprint 2's terminal spacing system per DEC-032). Cost ~$97 across three founder gates.

**What the sprint proved, worth carrying forward**: the replay stands on pacing and narration with the
styling subtracted — the founder's non-negotiable acceptance criterion — and every terminal line diffs
byte-clean against the corpus. Three consecutive fix rounds each satisfied their stated criterion and
disturbed an adjacent one, which is what produced DEC-032's shift from patching values to specifying
relationships, and DEC-033's two-gate model for Sprint 2.
