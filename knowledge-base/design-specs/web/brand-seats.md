...
**Every page seat is 6 × 9px.** Header lockup and all five section separators, one size.

The pennant is ~1:1.5 portrait (470:703 = 1:1.4957), so a mark set to a given width sets half again as
tall and the size is a judgment rather than an arithmetic. Three candidates were rendered against the real
`--text-label` wordmark and the real stencil tag, in both themes, and judged. An 8 × 8 square is the
comparison column because it is the neutral mark of the same family — same box, same `background-color`,
same seat — and it is what the eye calibrates a punctuation mark against:

| Candidate | Height vs. the label's cap height (8.4px) | Ink vs. an 8 × 8 square (64px²) | Verdict |
|---|---|---|---|
| 6 × 9 | 1.07× | 49px² — 23% lighter | **Ships.** Reads as punctuation; the notch is legible |
| 7 × 10.5 | 1.25× | 67px² — level | Heavier without being more legible |
| 8 × 12 | 1.43× | 87px² — 36% heavier | Badging. The mark out-weighs the caps it introduces |

Two things decided 6 × 9 rather than the arithmetic:
...
2. **A shape carries presence a neutral mark does not.** 6 × 9 puts 23% less ink on the page than an
   ...
   That is also the answer to DEC-031's stated worry: at 6 × 9 the mark is punctuation at every seat, so
   the page carries a brand shape six times over and carries no extra weight for it.

**Vertical placement is a relationship, not an offset: the mark's bottom edge sits on the baseline of
the text beside it.** Its top then clears the cap height by 0.6px — the slight overhang is what makes it
read as a marker set against the line rather than a bullet floating in it. Mechanism:
`align-self: baseline` on the flex item. **The tag's rendered block size must not change** when the
square becomes a pennant: baseline alignment inside a flex line can grow the line box, and the separator
construction is vertically centred in a grid row, so a 1px growth would shift every section tag off its
rule. That is an assertion, not a hope (§7).

**Mark to words is `--gap-hairline`, in both seats.** A mark this narrow makes 12px read wider than it
would beside a broader one, and 12px is what it stays: re-tuning an adjacent value to compensate for a
mark's proportions is the failure mode DEC-032 names, and *spacious* is the overriding constraint anyway.

## 3. Seat inventory

| Seat | Count | Treatment | Ships in this sprint |
|---|---|---|---|
| Header lockup | 1 | Pennant 6 × 9 + `MUSTER` + static rust underscore (§4) | Yes |
...
| # | Element | Spec |
|---|---|---|
| 1 | Pennant | 6 × 9px, `--accent`, `clip-path` per §1, bottom edge on the wordmark's baseline, `aria-hidden="true"` |
| 2 | Wordmark | `MUSTER` — `--text-label`, mono, uppercase, `--track-label`, `--ink`. Copy is Content's; the string is unchanged by this file |
| 3 | Underscore | A **drawn** rust bar: 1ch long, 2px thick, top edge 1px below the wordmark's baseline and bottom edge 3px below it. `--accent`. Static — it never blinks. `aria-hidden="true"` |
...
trailing letter-space (2.16px), which is exactly the next character position. **Do not cancel that
...
Five seats, one per `<h2>` stencil tag (`§02` … `§06`). The pennant is the tag's leading mark and the rest
of the construction — line, end-ticks, label — is `page-shell.md` §8's.

- 6 × 9px, `--accent`, `clip-path` per §1, `aria-hidden="true"`.
- Bottom edge on the tag label's baseline (§2).
- `--gap-hairline` between the mark and `§02 · WATCH IT SHIP`.
- The 1 × 9px `--hair` end-ticks either side of the tag share the mark's 9px, deliberately (§2).
...
(`page-shell.md` §11). Five separators, five pennants, and the header's is the sixth mark on the page.
...
## 8. The footer lockup — ruled out

`brand-guidelines.md` §4's seat table reads *"Header **and footer** lockup"*. DEC-031 enumerates four
rulings and the footer is not among them. **DEC-031 wins: the footer takes no pennant and no lockup.**

The precedence is the smaller half of the reason. The design reason is frequency. The header lockup plus
five separators already put the mark on the page six times; a seventh instance sits a few rhythm units
below the fifth separator's pennant and below §6's `curl`, and it does no work the page has not already
done. DEC-031's own guard against the mark becoming badging is stated as a scale limit — *punctuation at
8–12px, badging at 24px* — and the equivalent failure at the footer is repetition rather than size. The
footer's brand presence is its `--hair` top rule and the provenance line, which names Muster in the
founder's own first person (§5 copy, R7); a lockup above that line says the same thing twice, less well.
...
Four assertions cover these seats. Each is written as the relationship it protects rather than as the
number it happens to measure.

**The mark is the pennant at punctuation scale.** One check, three clauses: it measures 6 × 9, its height
**equals `.rule__tick`'s height**, and its `clip-path` is not `none`. Binding the height to the tick
rather than to the literal 9 is the part that carries weight — the two then move together or the check
fails, which is the shared measure §2 chose.
...
a real question rather than an obvious one (§4.1), so it is the one that most needs asserting.
...
**Decided here (the craft)**: 6 × 9 as the one page-seat size and the two reasons for it; the mark's
...
restated at a 16-unit viewBox with the tile's own coordinates; the footer ruled out.

**Measured, not derived**: every figure in §2's candidate table, and the underscore's placement — six
treatments were rendered at 6× against the real tokens in both themes before one was chosen.
