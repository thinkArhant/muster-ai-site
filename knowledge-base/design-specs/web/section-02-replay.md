...
Slot behavior: **desktop** — entries accumulate in the narration rail, all full-ink once revealed; the active entry carries a 2px `--accent` left bar + its beat tag; the rail never dims past entries (full-ink rule).

**The rail pages forward, and it lands the active entry's top.** The guarantee is not that the rail moves; it is that whenever an entry is the active one, the whole of it is inside the rail with rail beneath it — so the explanation that just arrived can be read where it arrived, not a slot later. The rail therefore holds still while the newest entry is already entirely inside it, and when the newest entry would fall outside it moves **once**, putting that entry's **top** on the rail's top, where it has the rail's full height to set in. Bringing the entry's **bottom** to the rail's bottom would also contain it and is the wrong rule: it delivers every new explanation flush on the fold of the scroll window, clipped for the length of its own reveal, and only readable one slot later — and the room a reader is given should be the room the entry is about to need. Three rules complete it: positions come from layout offsets rather than rendered rects, so the reveal's 4px rise cannot page the rail forward early on a transform that is about to disappear; movement is forward-only, so the rail is monotonic across a run and a reader who has scrolled ahead to read on is never yanked back; and the target clamps to the rail's own maximum, so the last entries — whose tops lie past anything the rail can scroll to — come to rest whole against the end of the list rather than demanding a position that does not exist. Instant position change, never an animated scroll (§5.1).

**Mobile** — the current entry renders in a fixed-height caption card that holds its place on screen for the whole playback, replaced per slot; the complete list renders in the end state. The card's height is budgeted for the longest slot (§7) and never reflows between slots.
...
| 3 | Terminal | `--surface`, 1px `--hair` border, sharp corners, registration `+` marks at two corners. Chrome bar: `--text-micro` label + live indicator (the shell's pulse dot — motion element 1's terminal instance) |
...
- **≥ `--bp-wide`**: two-column layout per wireframe. The narration rail **does** scroll its own overflow at every width from `--bp-wide` up — ten slots of prose set several times the terminal's height — and it follows the active entry by §6's paging rule. The precondition that rule rests on is that **no entry sets taller than the rail holding it**, since an entry that did could not be shown whole at any scroll position. `--bp-wide` itself is the tightest case in both directions at once — the rail is narrowest there, so the prose sets tallest — and the margin is measured rather than assumed.
